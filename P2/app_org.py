...
def change_pass_sec():
    ...
    if(password is not None and len(old_pass) > 0 and len(new_pass) > 0 and len(confirmed) > 0):
        ...
        if new_pass == confirmed and old_exist:
            ...
            if not(old_exist):
                return redirect(url_for('changePass_sec_page',error='Old Password dosent match'))
            return redirect(url_for('changePass_sec_page',error='Passwords do not match'))
    else:
        return redirect(url_for('changePass_sec_page',error='Username or password Incorrect'))
